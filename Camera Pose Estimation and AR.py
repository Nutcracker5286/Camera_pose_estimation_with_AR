...
if __name__ == '__main__':
    ...
    board_criteria =  cv.CALIB_CB_ADAPTIVE_THRESH + cv.CALIB_CB_NORMALIZE_IMAGE + cv.CALIB_CB_FAST_CHECK + cv.CALIB_CB_FILTER_QUADS
    ...
    # Prepare a 3D 'X' for simple AR
    x_lower = board_cellsize * np.array([[4, 2, 0], [5, 3, 0], [4, 4, 0], [5, 2, 0], [4, 3, 0], [5, 4, 0]])
    x_upper = board_cellsize * np.array([[4, 2, -1], [5, 3, -1], [4, 4, -1], [5, 2, -1], [4, 3, -1], [5, 4, -1]])
    ...
    while True:
        ...
        if success:
            ...
            xLine_lower, _ = cv.projectPoints(x_lower, rvec, tvec, K, dist_coeff)
            xLine_upper, _ = cv.projectPoints(x_upper, rvec, tvec, K, dist_coeff)
            cv.polylines(img, [np.int32(xLine_lower)], True, (255, 0, 0), 2)
            cv.polylines(img, [np.int32(xLine_upper)], True, (0, 0, 255), 2)
            for bot,top in zip(xLine_lower, xLine_upper):
                cv.line(img, np.int32(bot.flatten()), np.int32(top.flatten()), (0, 255, 0), 2)
